...
#### gkf: we modified all abs_path import as relative path import
from .vit_utils import IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD
from .helpers import load_pretrained, load_pretrained_kinetics, load_pretrained_imagenet, load_pretrained_CLIP_ViT
from .vit_utils import DropPath, to_2tuple, trunc_normal_

from ..xbert import BertAttention
from .. import grad_ckpt   # added by gkf
...
# we simply replace all the `LOGGER.info` as `print`, because we use a seperate logger in our OpenVoc project
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(x.shape)  # (B*T,3,224,224)
        x = self.proj(x)
        # print(x.shape)  # (B*T,768,14,14)
        W = x.size(-1)
        x = x.flatten(2).transpose(1, 2)
        # print(x.shape)  # (B*T,196,768)
        return x, T, W
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, return_all_tokens=False):
        ...
        x = torch.cat((cls_tokens, x), dim=1)  # (B*T, 196+1, 768)
...
class TimeSformer(nn.Module):
    def __init__(self, model_cfg, input_format='BGR', cross_attention_config=None, **kwargs):
        ...
        print(f'Initializing TimeSformer with img_size={self.img_size}, patch_size={self.patch_size}, num_frames={self.num_frames}')
    ...
    def load_state_dict(self, pretrained_ckpt_path):
        print('Loading TimeSformer checkpoints from {}'.format(pretrained_ckpt_path))
